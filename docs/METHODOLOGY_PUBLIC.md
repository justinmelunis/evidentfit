...
**Current Database**: ~30,000 carefully curated research papers, selected from 190,000+ candidate studies via comprehensive multi-supplement search (63 supplements), clinical/disease population filtering, quality filtering (reliability scoring), and iterative diversity filtering with minimum quotas, updated monthly.
...
- **Research on supplements for diseases**: Cancer, diabetes, heart disease, kidney disease, liver disease, neurological disorders, and other major diseases
- **Pediatric research**: Studies on children/adolescents under 18
- **Pregnancy/lactation research**: Studies on pregnant or breastfeeding individuals
- **Case reports**: Low-quality single-case studies

**Important exception**: We keep safety/adverse event studies even in clinical populations.
...
- Your profile maps to one of 270 pre-computed profile combinations × 27 supplements = 7,290 total reasoning entries
  ...
  - **3 Age Bins**: Young (18-29), adult (30-49), mature (50+) - pediatric populations excluded
  - **27 Supplements**: Core supplements with sufficient research for personalized recommendations
...
┌─────────────────────────────────────────────────────────────────────────────────┐
│  MODULE A: Data Ingestion (Monthly Job) - Non-agentic pipeline                │
│                                                                                 │
│  A1: Paper Fetcher              A2: Paper Indexer                              │
│  ┌─────────────────┐            ┌─────────────────┐                            │
│  │ PubMed Search   │ ──────────→│ • Join + Chunk  │                            │
│  │ • 63 supplements│  Parse     │ • Embeddings    │                            │
│  │ • Filter clinical│  & Filter │ • pgvector      │                            │
│  │ • Full-text fetch│            │ • Index update  │                            │
│  └─────────────────┘            └─────────────────┘                            │
│           │                              │                                     │
│           ▼                              ▼                                     │
│  ~30,000 Selected Papers          pgvector Database                           │
│  ┌─────────────────┐            ┌─────────────────┐                            │
│  │ • Full-text     │            │ • ~30,000 Papers │                            │
│  │ • Metadata      │            │ • Embeddings    │                            │
│  │ • Citations     │            │ • Chunks        │                            │
│  └─────────────────┘            └─────────────────┘                            │
└─────────────────────────────────────────────────────────────────────────────────┘
                                         ...
                                         ▼
┌─────────────────────────────────────────────────────────────────────────────────┐
│  AGENT D: Evidence Summarization (Monthly Job)                                 │
│                                                                                 │
...
│  DATA FLOW SUMMARY                                                             │
│                                                                                 │
│  Monthly: Module A → New Papers → Agent D → Updated Summaries                  │
│  As Needed: Agent C → Banking Cache Updates (GPT-4o-mini)                     │
│  Real-Time: User → Agent E (Chat) / Agent F (Stack) → Cached Banking + Live Search → Response │
│                                                                                 │
...
- **Process**: 
  - **A1: Paper Fetcher (get_papers)**: 
    - Multi-supplement PubMed search (63 supplements)
    - Clinical/disease population filtering (cancer, diabetes, pediatric, pregnancy)
    - Enhanced quota system: 10 best overall + 2 per goal combination per supplement
    - Parse XML and fetch full-text from PMC + Unpaywall
    ...
    - Abstract fallback for remaining 20-23% of papers
  - **A2: Paper Indexer (index_papers)**: 
    - Join papers with full-text data
    - Smart chunking for long papers (respects context limits)
    - Generate embeddings → Upload to pgvector
    - Update watermark for incremental updates
- **Output**: ~30,000 high-quality, diverse research papers with full-text content
...
- **Full-Text Coverage**: ≈77–80% (24-26k papers) via PMC + Unpaywall; abstracts for remaining 20-23%

**📊 Agent D (Evidence Summarization)**  
- **Purpose**: Create public-facing supplement summaries
- **Frequency**: Monthly after Module A completes
- **Process**: Analyze papers by supplement → Rank by quality → Generate summaries with Level 1 banking grades
...
- **Process**: GPT-4o-mini reads papers → Generates structured summaries → Assigns grades → Generates profile-specific explanations
- **Output**: 162 evidence grades (6 goals × 27 supplements) + 7,290 personalized reasoning entries (270 profiles × 27 supplements)
- **Cost**: $5.32 per run (~$21/year for quarterly updates)

**💬 Agent E (Research Chat)**
- **Purpose**: Deliver real-time research Q&A with citations
- **Frequency**: Always running, responds to user requests
- **Process**: Search papers → Synthesize evidence → Compose response with citations
- **Output**: Evidence-based answers with direct PubMed links

**🎯 Agent F (Stack Builder)**
- **Purpose**: Deliver real-time personalized supplement stack recommendations
- **Frequency**: Always running, responds to user requests
- **Process**: Load banking → Apply safety → Generate personalized stack with dosing
...
Our LLM Research Agent (GPT-4o-mini via Azure AI Foundry) doesn't just count papers—it **reads and analyzes** research content to make evidence-based determinations:

**🚀 Cloud-Based Processing:**
- **Azure AI Foundry**: Managed cloud processing with high reliability
- **Cost-Effective**: GPT-4o-mini optimized for cost ($5.32 per banking run)
- **Structured Analysis**: Generates detailed summaries with key findings, methodology, and evidence grades
- **Fast Processing**: Parallel execution processes 162 Level 1 combinations + 7,290 Level 2 combinations (270 profiles × 27 supplements) in 20-30 minutes

**📚 Enhanced Paper Analysis Process:**
1. **Retrieval**: Fetch top relevant studies for each supplement × goal combination
2. **Structured Analysis**: GPT-4o-mini reads study abstracts and generates structured summaries with:
...
**🔬 Example Analysis:**
*"Creatine for Strength": LLM reads 67 studies → generates structured summaries → finds multiple meta-analyses showing 5-15% strength gains → consistent across RCTs → clinically meaningful effects → assigns Grade A*

**💰 Cost Benefits:**
- **Cost-Effective**: $5.32 per run (~$21/year for quarterly updates)
- **Fast**: 20-30 minutes with parallel execution
...
- **Source**: **LLM Research Agent** (GPT-4o-mini) reads and analyzes research papers for goal-specific outcomes
...
- **Supporting Publications**: Each evidence grade includes top 3 supporting research papers with full metadata
- **Updated**: Monthly when new research is ingested
- **Example**: "Creatine for strength = Grade A (LLM analyzed 67 studies, generated structured summaries, found consistent 5-15% strength gains across multiple meta-analyses) + 3 key publications"

#### **Level 2: Profile-Specific Reasoning Banking**
- **What**: Personalized "why" explanations based on demographics and research
- **Cached**: 270 profile combinations (6 goals × 5 weight bins × 3 sexes × 3 age bins)
- **Source**: **LLM Research Agent** (GPT-4o-mini) analyzes retrieved papers and generates profile-specific reasoning
...
**270 Pre-Computed Profile Combinations:**
- Every combination of goal × weight bin × sex × age bin (pediatric excluded)
- Each profile × supplement combination contains personalized reasoning with evidence grades
- 270 profiles × 27 supplements = 7,290 total reasoning entries
...
**Storage Efficiency:**
- 7,290 reasoning entries (270 profiles × 27 supplements) × ~250 bytes each = ~1.8MB total storage
